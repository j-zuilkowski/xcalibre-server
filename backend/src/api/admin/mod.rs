...
async fn update_check(
    ...
    *cached_update_check().write().await = Some(cached);
    ...
    // Auto-update: if the feature is enabled and auto_update is on, apply now.
    if response.update_available
        && state.config.updater.enabled
        && state.config.updater.auto_update
    {
        if let Some(ref latest) = response.latest_version {
            tracing::info!(version = %latest, "auto-update: applying");
            match crate::updater::apply(&state.config.updater, latest, false).await {
