...
async fn test_update_check_returns_current_version() {
    ...
    Mock::given(method("GET"))
        .and(path("/repos/xcalibre-server/xcalibre-server/releases/latest"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "tag_name": "v999.0.0",
            "html_url": "https://github.com/xcalibre-server/xcalibre-server/releases/tag/v999.0.0"
        })))
        .mount(&mock_server)
        .await;
    ...
    std::env::set_var(
        "XCS_RELEASES_URL",
        format!(
            "{}/repos/xcalibre-server/xcalibre-server/releases/latest",
            mock_server.uri()
        ),
    );
    ...
    std::env::remove_var("XCS_RELEASES_URL");
    clear_update_check_cache().await;
    ...
    assert_eq!(
        body["release_url"],
        "https://github.com/xcalibre-server/xcalibre-server/releases/tag/v999.0.0"
    );
}
...
async fn test_update_check_github_unreachable_returns_503_gracefully() {
    ...
    std::env::set_var(
        "XCS_RELEASES_URL",
        "http://127.0.0.1:9/repos/xcalibre-server/xcalibre-server/releases/latest",
    );
