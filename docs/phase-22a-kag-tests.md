# Phase 22a — KAG Knowledge Graph Layer: Failing Tests

## Context

Rust 2021, Axum 0.7, sqlx 0.7. TDD — write failing tests first.
Working dir: `~/Documents/localProject/xcalibre-server`

Phase 21 complete (v2.4.0). Phase 30 (v2.5.0) planned separately.
This is Phase 22-KAG, targeting v2.6.0.
...
Three new endpoints:
- `POST /api/v1/graph/triples` — Merlin writes session triples (auth required; always accepted)
- `GET /api/v1/graph/traverse` — BFS from an anchor entity, with domain and source filters
- `GET /api/v1/search/enriched` — hybrid chunk search + graph traversal in one fused response

New modules: `backend/src/graph/mod.rs`, `backend/src/graph/traverse.rs`,
`backend/src/graph/extract.rs`.

New migration: `backend/migrations/sqlite/0034_knowledge_graph.sql` (and mariadb equivalent).

Write tests only. All must FAIL before implementation.

---

## Write to: `backend/tests/test_kag.rs`
...
Expected: compile errors on missing routes and types. That is the correct failing state.
...
```bash
git add backend/tests/test_kag.rs
git commit -m "Phase 22a — KAG knowledge graph: failing tests"
```
